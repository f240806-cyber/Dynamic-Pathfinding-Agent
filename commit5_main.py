#Add metrics dashboard and finalize GUI
import random
import heapq
import math
import time
...
def draw_panel(screen, algorithm, h_type, edit_mode, dynamic_mode, nodes_visited, path_cost, exec_time, status):
    ...
    for label, val, col in [
        ("Nodes", str(nodes_visited), CYAN),
        ("Path Cost", str(path_cost), CYAN),
        ("Time", f"{exec_time:.1f}ms", CYAN),
        ("Status", status, GREEN if "Found" in status or "Reached" in status else RED if "No" in status else YELLOW)
    ]:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for event in pygame.event.get():
                ...
                if mx < GRID_W:
                    ...
                    if 0 <= r2 < rows and 0 <= c2 < cols:
                        ...
                        if edit_mode == "wall" and pos != start and pos != goal:
                            placing = WALL if grid[r2][c2] != WALL else EMPTY
                            grid[r2][c2] = placing
                            path, visited, frontier = [], set(), set()
                            status = "Ready"
                    ...
                    btn_rects = draw_panel(screen, algorithm, h_type, edit_mode, dynamic_mode, nodes_visited, path_cost, exec_time, status)
                    for lbl, rect in btn_rects.items():
            ...
            if event.type == pygame.MOUSEMOTION:
                if event.buttons[0] and event.pos[0] < GRID_W and edit_mode == "wall" and placing is not None:
                    ...
                    c2 = mx // cell_size
                    r2 = my // cell_size
                    if 0 <= r2 < rows and 0 <= c2 < cols and (r2,c2) != start and (r2,c2) != goal:
                        grid[r2][c2] = placing
        ...
        draw_panel(screen, algorithm, h_type, edit_mode, dynamic_mode, nodes_visited, path_cost, exec_time, status)
        pygame.display.flip()
