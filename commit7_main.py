#Add keyboard shortcuts and spawn probability tuning
import random
import heapq
import math
import time
...
class App:
        ...
        for lbl, rect in self.btn_rects.items():
            if rect.collidepoint(pos):
                if lbl == "Generate Map":
                    ...
                    self.spawn_prob = min(round(self.spawn_prob + 0.01, 2), 0.2)
                elif lbl == "Prob-":
                    self.spawn_prob = max(round(self.spawn_prob - 0.01, 2), 0.01)
                elif lbl == "A* Search":
                    self.algorithm = "astar"
                    self.reset_search()
                elif lbl == "Greedy BFS":
    ...
    def draw_panel(self):
        ...
        half = (PANEL_W-20)//2
        for lbl, col, hint in [("Speed-",BTN,str(self.anim_speed)),("Speed+",BTN,"spd")]:
            r = pygame.Rect(10 if lbl=="Speed-" else 10+half+4, y, half-2, 24)
            pygame.draw.rect(panel, col, r, border_radius=4)
            panel.blit(font_sm.render(lbl, True, WHITE),(r.x+4,r.y+5))
            self.btn_rects[lbl] = pygame.Rect(GRID_W+r.x, r.y, r.w, r.h)
        panel.blit(font_sm.render(f"Spd:{self.anim_speed}", True, CYAN),(10,y+4))
        y+=28

        for lbl, col in [("Prob-",BTN),("Prob+",BTN)]:
            r = pygame.Rect(10 if lbl=="Prob-" else 10+half+4, y, half-2, 24)
            pygame.draw.rect(panel, col, r, border_radius=4)
            panel.blit(font_sm.render(lbl, True, WHITE),(r.x+4,r.y+5))
            self.btn_rects[lbl] = pygame.Rect(GRID_W+r.x, r.y, r.w, r.h)
        panel.blit(font_sm.render(f"Prob:{self.spawn_prob:.2f}", True, CYAN),(10,y+4))
        y+=32
        ...
        y+=6
        pygame.draw.line(panel,(60,60,100),(5,y),(PANEL_W-5,y)); y+=6
        ...
        y+=6
        panel.blit(font_sm.render("SPACE=Run  M=Agent", True, GRAY),(10,y)); y+=14
        panel.blit(font_sm.render("R=Regen  D=Dynamic", True, GRAY),(10,y)); y+=14
        panel.blit(font_sm.render("1=A*  2=GBFS", True, GRAY),(10,y))
