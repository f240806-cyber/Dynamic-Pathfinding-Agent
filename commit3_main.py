#Add Pygame GUI and visualization
import random
import heapq
import math
import pygame
...
def draw_panel(screen, algorithm, heuristic_type, edit_mode, dynamic_mode, nodes_visited, path_cost, exec_time, status):
    ...
    btn_labels = [("Generate Map",GREEN),("Run & Animate",BLUE),("Move Agent",ORANGE),("Clear Search",GRAY)]
    btn_rects = {}
    for lbl, col in btn_labels:
    ...
    for label, val, col in [("Nodes",str(nodes_visited),CYAN),("Path Cost",str(path_cost),CYAN),("Time",f"{exec_time:.1f}ms",CYAN),("Status",status,GREEN if "Found" in status else RED if "No" in status else CYAN)]:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        draw_panel(screen, algorithm, heuristic_type, edit_mode, dynamic_mode, nodes_visited, path_cost, exec_time, status)
        pygame.display.flip()
